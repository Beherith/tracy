...
```text
AI Client (Copilot, stdio/SSE)
  |
  | Standard MCP via FastMCP
  v
bar_tracy_bridge.py
  |-- BAR TCP Adapter (TCP <-> JSON-RPC) -> BAR MCP TCP 127.0.0.1:23452
  |-- Tracy Client (HTTP -> SSE)         -> Tracy MCP SSE 127.0.0.1:47380
```
...
#### Phase 1: BAR TCP Adapter (solves the transport problem) [done] DONE

**Implemented:** `bar_tracy_bridge.py` - Phase 1 complete.

**1.1 - `BarTcpClient` class** (inside the single file) [done]
...
**1.2 - BAR tool forwarding** (inside the single file) [done]
...
#### Phase 2: Tracy Integration [done] DONE

**Implemented:** `bar_tracy_bridge.py` - Phase 2 complete.

**2.1 - Tracy HTTP client** (`TracyHttpClient`) [done]
...
**2.2 - Tracy auto-start** (`TracyAutoStart`) [done]
...
#### Phase 3: Profile Tools (the core workflow) [done] DONE

**Implemented:** `bar_tracy_bridge.py` - Phase 3 complete.

**3.1 - `ProfileCollector` class** [done]
...
**3.2 - Four MCP tools registered on the bridge server** [done]
...
- Diff output includes direction text (up/down/flat), percentage change, and before->after avg comparison

#### Phase 4: Entrypoint [done] DONE

**Implemented:** `bar_tracy_bridge.py` + `start_bridge.sh` - Phase 4 complete.

**4.1 - Main server** (inside `bar_tracy_bridge.py`) [done]
...
**4.2 - Launcher** - `extra/mcp/start_bridge.sh` [done]
...
13. **Readable + JSON output format** - profile results include a human-readable table (sorted by total time, capped at 50 zones, times in us) followed by full JSON for programmatic access
